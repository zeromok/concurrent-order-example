...
## V1: 멀티스레드 도입
### 목표
-  V0의 불변조건(예: `stock == initialStock - completedOrderCount`)이 **멀티스레드 환경에서 깨질 수 있음**을 재현한다.

### 구현한 부분
- 여러 스레드가 동시에 `processOrder()`를 호출하는 테스트/시나리오를 추가한다.
- 동일 상품에 대해 동시에 재고를 차감하면서 레이스 컨디션을 유도한다.
...
- V2에서 동시 시작을 강제(예: latch)해 재현을 더 결정적으로 만든다.
