...
## V1, V2: 멀티스레드
### 목표
- V0 불변조건(예: `stock == 초기 재고 - 성공 주문 수`)이 **멀티스레드 환경에서 깨질 수 있음**을 재현한다(V1).
- V1 재현이 스케줄링 운에 흔들리지 않도록, 스레드 생명주기 제어로 **동시 시작/종료 대기**를 고정한다(V2).

### 구현한 부분
- 여러 스레드가 동시에 `processOrder()`를 호출하도록 만든다.
- 동일 상품에 대해 동시에 재고를 차감하면서 **race condition / lost update**를 유도한다.
- 관측 예시:
    - `주문 성공=28`인데 `실제 재고=96`, `예상 재고=72`처럼 숫자가 어긋날 수 있다

### 예상되는 실패
- 성공 주문 수(`COMPLETED`)와 재고 감소량이 논리적으로 불일치할 수 있다.  - 예: `completed = 7`인데 `stock(actual) = 90`처럼, 기대값 `stock(expected)=93`과 다르게 나올 수 있음- 이는 재고 차감이 “원자적 연산”이 아니고, 동시 갱신에서 **lost update / race condi

### 정리
- V1의 재현은 OS 스케줄링에 따라 흔들릴 수 있다(가끔만 깨짐).
- V2에서 동시 시작을 강제(예: latch)해 재현을 더 결정적으로 만들었다.
